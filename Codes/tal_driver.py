...
def listener():
	...
	if(joint1_init_pos==0):
		...
		while(True):
			...
			if(not (sensor_val==-1)):
				if(pre_sensor_val==-1):
					...
					print('The sensor value is %d' %sensor_val)
					print(sensor_val)
		...
		while(True):
			tm = time.clock()
			print('Sending Pulse %d' %pulses[0])
			#Sending commands to arduino
			write_Int16(pulses[0],ser)
			write_Int16(pulses[1],ser)
			write_Int16(pulses[2],ser)
	...
	if(joint2_init_pos==0):
		...
		while(True):
			tm = time.clock()
			print('Sending Pulse %d' %pulses[1])
			#Sending commands to arduino
			write_Int16(pulses[0],ser)
			write_Int16(pulses[1],ser)
			write_Int16(pulses[2],ser)
...
#Homing of JOINT 3-----------------------------------------------33333333333333333333333333333333333-------------------------------
	...
	if(joint3_init_pos==0):
		...
		while(True):
			tm = time.clock()
			print('Sending Pulse %d' %pulses[2])
			#Sending commands to arduino
			write_Int16(pulses[0],ser)
			write_Int16(pulses[1],ser)
			write_Int16(pulses[2],ser)
	...
	while(True):
		...
		if(not (sensor_val==-1)):
			if(pre_sensor_val==-1):
				...
				if(sensor_val&0b00000010==0b00000000 and pre_sensor_val&0b00000010==0b00000000):
					print('Joint 4 2')
					dir_ena=0
					pulses[3] = 0
					break
...
#Homing of JOINT 5-----------------------------------------------55555555555555555555555555555555555-------------------------------
	...
	#-------------------------------------------------HOMING DONE-----------------------------------------------------------------------------
	initial_pulse_req = (35*PPR[0]*GEAR_RATIO[0]*DEG2RAD,16*PPR[1]*GEAR_RATIO[1]*DEG2RAD,(38.5+90)*PPR[2]*GEAR_RATIO[2]*DEG2RAD,(4+90)*PPR[3]*GEAR_RATIO[3]*DEG2RAD,20*PPR[4]*GEAR_RATIO[4]*DEG2RAD)
	pulses = [0,0,0,0,0]
	cur_pul = [0,0,0,0,0]
	num_pul = 0
	...
	while(True):
		...
		if(cur_pul[0]<initial_pulse_req[0]):
			...
			print('The current pulsex now is %d' %cur_pul[0])
		if(cur_pul[1]<initial_pulse_req[1]):
			pulses[1] = 5		
			cur_pul[1] = cur_pul[1]+10
			print('The current pulsey now is %d' %cur_pul[1])
	...
	while not rospy.is_shutdown():
		...
		if(n>0):
			...
			else:				#Clearing buffer in case of wrong data from arduino
				ser.read(n)
